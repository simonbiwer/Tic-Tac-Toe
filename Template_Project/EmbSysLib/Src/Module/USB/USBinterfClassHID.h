...
#define _HW_USB_INTERF_CLASS_HID_C
...
#define HID_REPORT_NUM      16

//*******************************************************************
class cHwUSBinterfClassHID : public cHwUSBinterf
{
  public:
    //---------------------------------------------------------------
    /*! 
    */
    cHwUSBinterfClassHID( cHwUSB &usb, 
                          BYTE    configId, 
                          BYTE    interfId
                        );
...
//    virtual int request( BYTE epId, cDataPointer &dataPtr );
    virtual int requestCtrlIN( cDataPointer &dataPtr, BYTE request, WORD value, WORD length);
    virtual int requestCtrlOUT( cDataPointer &dataPtr, BYTE request, WORD value, WORD length);
  ...
  private:
    ...
    DWORD HID_GetReport   (cUSBctrlSetup &setup, BYTE *buffer);
    DWORD HID_SetReport   (cUSBctrlSetup &setup, const cDataPointer &buffer) ;
    DWORD HID_GetIdle     (cUSBctrlSetup &setup, BYTE *buffer);
DWORD HID_SetIdle     (WORD value);
    DWORD HID_GetProtocol (BYTE *buffer);
    DWORD HID_SetProtocol (cUSBctrlSetup &setup);
